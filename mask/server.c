...
void connection_handler(int socket_desc, struct sockaddr_in* client_addr) {
    ...
    // Welcome message REMOVE IF NOT REQUESTED
    // send welcome message
    sprintf(buf, "Hi! I'm an server. You are %s talking on port %hu. I will send you back whatever"
            " you send me. I will stop if you send me %s :-)\n", client_ip, client_port, quit_command);
    msg_len = strlen(buf);

    // This send the data
    send_data(socket_desc,buf,msg_len);
...
void mthreadServer(int server_desc) {
    ...
    while (1) {
        ...
        debug_printf("Incoming connection accepted...\n");
        ...
        debug_printf("New thread created to handle the request!\n");
        
        // Attenzione al detach può essere non richiesto dall'esercizio
...
void mprocServer(int server_desc) {
    ...
    while (1) {
        ...
        // Importante resettare la struct per preparare una nuova connessione
...
int main(int argc, char* argv[]) {
    ...
    // Ricordarsi che thread_connection_handler e la struct servono solo nel thread
    // Eliminarle nel caso
