...
unsigned char* volatile output_img;
int width, height;
...
void session(int sock_fd) {
    ...
    if (output_img == NULL) {
        fprintf(stderr, "Impossibile allocare buffer della dimensione desiderata!\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (thread_index = 0; thread_index < NUM_THREADS; ++thread_index) {
        ret = sem_wait(&semaphores[thread_index]);
        ERROR_HELPER(ret, "Errore nella wait sul semaforo");
    }

    char* write_buf;
    int output_file_bytes = pgm_to_ascii(output_img, width, height, &write_buf);
